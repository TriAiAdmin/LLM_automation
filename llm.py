...
def get_openai_response(base64_image,prompt):
    ...
    if response.status_code == 200:
        ...
        return company_code, invoice_type, invoice_no, supplier_name, supplier_address, supplier_telephone, supplier_email, supplier_website, supplier_vat_reg_no, supplier_svat_reg_no, supplier_business_reg_no, po_number, delivery_note_number, invoice_date, currency, invoice_amount, invoice_tax_amount, comment
    else:
...
def process_files(batch_files):
    ...
    for file_name in batch_files:
        ...
        for image in images:
            ...
            company_code, invoice_type, invoice_no, supplier_name, supplier_address, supplier_telephone, supplier_email, supplier_website, supplier_vat_reg_no, supplier_svat_reg_no, supplier_business_reg_no, po_number, delivery_note_number, invoice_date, currency, invoice_amount, invoice_tax_amount, comment = get_openai_response(base64_image,prompt)
            if company_code or invoice_type or invoice_no or supplier_name or supplier_address or supplier_telephone or supplier_email or supplier_website or supplier_vat_reg_no or supplier_svat_reg_no or supplier_business_reg_no or po_number or delivery_note_number or invoice_date or currency or invoice_amount or invoice_tax_amount:
            
                if supplier_name and supplier_address: 
                    vendor_code =  find_best_match(supplier_name.lower(), supplier_address.lower(), vendor_master)
                else:
                    vendor_code = None
